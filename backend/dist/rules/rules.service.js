    ...
    run(report) {
        ...
        for (const loan of report.accounts) {
            ...
            if (loan.status === 'CLOSED' && loan.dpd > 0) {
                issues.push(this.issue('CLOSED_ACCOUNT_WITH_DPD', loan.bureau, 'HIGH', 'Loan is closed but DPD is reported'));
            }
            // RULE 2: Settled but active
            if (loan.settlement === 'SETTLED' && loan.status === 'ACTIVE') {
                issues.push(this.issue('SETTLED_SHOWING_ACTIVE', loan.bureau, 'HIGH', 'Settled account still active'));
            }
        }
        return issues;
    }
